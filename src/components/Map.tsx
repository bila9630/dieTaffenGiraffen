  ...
  const initializeMap = (token: string) => {
    ...
    try {
      ...
      map.current.on('style.load', () => {
        ...
        // Add a layer to mask everything outside a 100km buffer around Austria
        // Austria bounds with ~100km buffer: [8.2, 45.5, 18.5, 49.9]
        ...
        // Add a transparent "window" showing Austria + 100km buffer
        map.current?.addSource('austria-buffer', {
          type: 'geojson',
          data: {
            type: 'Feature',
            properties: {},
        ...
        map.current?.addLayer({
          id: 'austria-buffer-mask',
          type: 'fill',
          source: 'austria-buffer',
          paint: {
            'fill-color': '#000000',
            'fill-opacity': 0
          }
        ...
        // Mask the background outside the buffer using a large polygon with a hole
        map.current?.addSource('outside-buffer-mask', {
          type: 'geojson',
          data: {
            type: 'Feature',
            properties: {},
            geometry: {
              ...
              coordinates: [
                ...
                // Inner ring (hole for Austria buffer)
                [
                  [8.2, 45.5],
                  [8.2, 49.9],
                  [18.5, 49.9],
        ...
        map.current?.addLayer({
          id: 'outside-buffer-fill',
          type: 'fill',
          source: 'outside-buffer-mask',
          paint: {
            'fill-color': '#000000',
            'fill-opacity': 1.0
          }
